...
name: refactoring
description: |
    Use when identifying code smells and applying systematic refactoring techniques — based on Martin Fowler's "Refactoring."
    USE FOR: code smell identification, refactoring technique selection, safe code transformation, incremental improvement workflows, legacy code improvement
    DO NOT USE FOR: clean code principles (use clean-code), design pattern application (use dev/design-patterns), architecture restructuring (use dev/architecture)
license: MIT
metadata:

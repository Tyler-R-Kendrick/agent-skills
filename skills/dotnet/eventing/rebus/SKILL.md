...
name: rebus
description: |
  Use when building message-driven .NET applications with Rebus, a lean and extensible service bus supporting multiple transports.
  USE FOR: lightweight service bus messaging, pub/sub with RabbitMQ or Azure Service Bus, saga orchestration, message routing, deferred messages, transport-agnostic messaging
  DO NOT USE FOR: in-process mediator pattern (use mediatr), enterprise service bus with commercial support (use nservicebus), actor concurrency model (use akka-net)
license: MIT
metadata:
  displayName: "Rebus"

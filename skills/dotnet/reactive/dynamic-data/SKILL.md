...
name: dynamic-data
description: >
  USE FOR: Reactive collection management using observable caches and lists with LINQ-style
  operators for filtering, sorting, grouping, transforming, and binding to UI collections.
  DO NOT USE FOR: Simple ObservableCollection scenarios, non-reactive LINQ queries, or
  server-side data access (use Entity Framework or Dapper).
license: MIT
metadata:

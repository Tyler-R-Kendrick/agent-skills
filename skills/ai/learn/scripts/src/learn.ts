...
type LearningRecord = Required<Pick<LearningInput,
  "title" | "category" | "trigger" | "correction" | "preferredStrategy" |
  "generalizedRule" | "zpdScaffold" | "evidence" | "scope" | "confidence" |
  "avoid" | "status"
>> & {
  id: string;
  createdAt?: string;
...
function deterministicId(input: LearningInput, category: string): string {
...
function evidenceUri(evidence: EvidenceInput): string {
  return typeof evidence === "string" ? evidence : evidence.uri;
}

function evidenceLabel(evidence: EvidenceInput): string | undefined {
  return typeof evidence === "string" ? undefined : evidence.label;
}

function validateInput(value: unknown): LearningInput {
  ...
  return input as LearningInput;
}

function normalizeRecord(input: LearningInput): LearningRecord {
...
function validateEvidenceUri(uri: string): void {
  let parsed: URL;
  try {
    parsed = new URL(uri);
  } catch {
...
function buildTurtle(record: LearningRecord): string {
  const lines = [
...
async function ensureHome(home: string): Promise<void> {
  await mkdir(join(home, ".steering"), { recursive: true });
}

async function walkTurtleFiles(root: string): Promise<string[]> {
  if (!existsSync(root)) {
    return [];
  }
...
function extractString(content: string, predicate: string): string | undefined {
...
function parseRecordFromTurtle(home: string, filePath: string, content: string): LearningRecord | undefined {
  ...
  return {
    ...
    avoid: [],
    status,
    relativePath: relative(home, filePath).split(sep).join("/"),
  };
}

async function regenerateIndex(home: string): Promise<{ entries: number; path: string }> {
  await ensureHome(home);
  const files = await walkTurtleFiles(join(home, ".steering"));
  ...
  lines.push(
    ...
    "## Instruction Hook",
    "",
    "When available, agents should read this index before substantial work, then load only matching linked RDF entries.",
    "",
  );

  const indexPath = join(home, "STEERING.md");
...
async function commandInit(home: string): Promise<Record<string, unknown>> {
  await ensureHome(home);
...
async function commandAdd(home: string, inputPath: string | undefined): Promise<Record<string, unknown>> {
  if (!inputPath) {
  ...
  const ttlPath = join(home, ".steering", record.category, `${record.id}.ttl`);
  await mkdir(dirname(ttlPath), { recursive: true });
  await writeFile(ttlPath, buildTurtle(record), "utf8");
...
async function commandLint(home: string): Promise<Record<string, unknown>> {
  const errors: string[] = [];
  const warnings: string[] = [];
  const steeringPath = join(home, "STEERING.md");
  const steeringDir = join(home, ".steering");
  ...
  const ttlRelativePaths = new Set(files.map((file) => relative(home, file).split(sep).join("/")));

  for (const file of files) {
    const content = await readFile(file, "utf8");
    const rel = relative(home, file).split(sep).join("/");
    const id = extractId(content);
  ...
  if (existsSync(steeringPath)) {
    const index = await readFile(steeringPath, "utf8");
...
function commandTemplate(): Record<string, unknown> {
  return {
...
function commandHelp(): Record<string, unknown> {
  return {
    status: "success",
    commands: {
...
main().catch((error: unknown) => {
  ...
  stderr.write(`${JSON.stringify({ status: "error", error: message })}\n`);
  exit(1);
});
